...
readConfigHostPathUUID() {
	...
	if [[ -n "${configPath}" ]]; then
		...
		if [[ $(echo "${fallback}"|jq -r .dest) == 33297 ]]; then
			currentPath=$(echo "${path}" | awk -F "[w][s]" '{print $1}')
		elif [[ $(echo "${fallback}"|jq -r .dest) == 33298 ]]; then
			currentPath=$(echo "${path}" | awk -F "[t][c][p]" '{print $1}')
		elif [[ $(echo "${fallback}"|jq -r .dest) == 33299 ]]; then
...
initTLSNginxConfig() {
	...
	if [[ -n "${currentHost}" ]]; then
		...
		if [[ "${historyDomainStatus}" == "y" ]]; then
			...
			echo
			echoContent yellow "请输入要配置的域名 例：g.com --->"
			read -r -p "域名:" domain
		fi
	else
		echo
	...
	if [[ -z ${domain} ]]; then
		...
		echo "server {listen 80;listen [::]:80;server_name ${domain};root /usr/share/nginx/html;location ~ /.well-known {allow all;}location /test {return 200 'fjkvxmc7len';}}" >/etc/nginx/conf.d/alone.conf
		# 启动nginx
		handleNginx start
		echoContent yellow "\n检查IP是否设置为当前VPS"
		checkIP
		...
		echoContent yellow "\n检查Nginx是否正常访问"
		sleep 0.5
		domainResult=$(curl -s "${domain}/test" --resolve "${domain}:80:${pingIP}" | grep fjkvxmc7len)
...
server {
    ...
    location /test {return 200 'fjkvxmc7len';}
}
EOF
}
...
nginxBlog() {
	...
	if [[ -d "/usr/share/nginx/html" && -f "/usr/share/nginx/html/check" ]]; then
		...
		if [[ "${nginxBlogInstallStatus}" == "y" ]]; then
			...
			wget -q -P /usr/share/nginx https://raw.githubusercontent.com/wusansan2015/v2ray-agent/master/fodder/blog/unable/html${randomNum}.zip >/dev/null
...
EOF
	# VLESS_TCP_TLS/XTLS
	# 回落nginx
	local fallbacksList='{"dest":33300,"xver":0},{"alpn":"h2","dest":33302,"xver":0}'

#	if echo "${selectCustomInstallType}" | grep -q 4 || [[ "$1" == "all" ]]; then
#		# 回落trojan-go
#		fallbacksList='{"dest":33296,"xver":1},{"alpn":"h2","dest":33302,"xver":0}'
#	fi

if [[ -n $(echo "${selectCustomInstallType}" | grep 4) || "$1" == "all" ]]; then
#		fallbacksList=${fallbacksList}',{"path":"/'${customPath}'tcp","dest":33298,"xver":1}'
		fallbacksList='{"dest":33296,"xver":1},{"alpn":"h2","dest":33302,"xver":0}'
...
EOF
	...
	if echo "${selectCustomInstallType}" | grep -q 1 || [[ "$1" == "all" ]]; then
		fallbacksList=${fallbacksList}',{"path":"/'${customPath}'ws","dest":33297,"xver":1}'
...
#	fi
	...
	if echo "${selectCustomInstallType}" | grep -q 3 || [[ "$1" == "all" ]]; then
		fallbacksList=${fallbacksList}',{"path":"/'${customPath}'vws","dest":33299,"xver":1}'
...
#		fallbacksList=${fallbacksList}',{"alpn":"h2","dest":33301,"xver":0}'
...
#	if echo "${selectCustomInstallType}" | grep -q 4 || [[ "$1" == "all" ]]; then
#		# 回落trojan-go
#		fallbacksList='{"dest":33296,"xver":0},{"alpn":"h2","dest":33302,"xver":0}'
#	fi
...
EOF
	...
	if echo "${selectCustomInstallType}" | grep -q 5 || [[ "$1" == "all" ]]; then
#		fallbacksList=${fallbacksList}',{"alpn":"h2","dest":33302,"xver":0}'
...
EOF
			...
			echoContent green "    https://com/v1/create-qr-code/?size=400x400&data=vless%3A%2F%2F${VLESSID}%40${host}%3A${port}%3F${encryption}%3Dnone%26security%3Dxtls%26type%3Dtcp%26${host}%3D${host}%26headerType%3Dnone%26sni%3D${host}%26flow%3Dxtls-rprx-direct%23${VLESSEmail}\n"
...
EOF
			...
			echoContent green "    https://com/v1/create-qr-code/?size=400x400&data=vless%3A%2F%2F${VLESSID}%40${host}%3A${port}%3F${encryption}%3Dnone%26security%3Dxtls%26type%3Dtcp%26${host}%3D${host}%26headerType%3Dnone%26sni%3D${host}%26flow%3Dxtls-rprx-splice%23${VLESSEmail}\n"
...
EOF
			...
			echoContent green "    https://com/v1/create-qr-code/?size=400x400&data=vless%3a%2f%2f${VLESSID}%40${host}%3a${port}%3fsecurity%3dtls%26encryption%3dnone%26host%3d${host}%26headerType%3dnone%26type%3dtcp%23${VLESSEmail}\n"
		fi
...
EOF
		echoContent green "    https://com/v1/create-qr-code/?size=400x400&data=vmess://${qrCodeBase64Default}\n"
...
#		echoContent green "    https://com/v1/create-qr-code/?size=400x400&data=vmess%3A%2F%2Fhttp%2Btls%3A${id}-0%40add%3A${port}%2F%3Fpath%3D%2F${path}%26tlsServerName%3D${host}%26alpn%3Dhttp1.1%23%24%7B${ps}%7D\n"
...
EOF
		...
		echoContent green "    https://com/v1/create-qr-code/?size=400x400&data=vless%3A%2F%2F${VLESSID}%40${add}%3A${port}%3Fencryption%3Dnone%26security%3Dtls%26type%3Dws%26host%3D${host}%26sni%3D${host}%26path%3D%252f${path}%23${VLESSEmail}"
...
EOF
		...
		echoContent green "    https://com/v1/create-qr-code/?size=400x400&data=vless%3A%2F%2F${VLESSID}%40${add}%3A${port}%3Fencryption%3Dnone%26security%3Dtls%26type%3Dgrpc%26host%3D${host}%26serviceName%3D${path}%26path%3D${path}%26sni%3D${host}%26alpn%3Dh2%23${VLESSEmail}"
...
EOF
		...
		echoContent green "    https://com/v1/create-qr-code/?size=400x400&data=trojan%3a%2f%2f${id}%40${host}%3a${port}%3fpeer%3d${host}%26sni%3d${host}%26alpn%3Dhttp1.1%23${host}_trojan\n"
...
EOF
		...
		echoContent green "    https://com/v1/create-qr-code/?size=400x400&data=trojan%3a%2f%2f${id}%40${add}%3a${port}%3fallowInsecure%3d0%26peer%3d${host}%26plugin%3dobfs-local%3bobfs%3dwebsocket%3bobfs-host%3d${host}%3bobfs-uri%3d${path}%23${host}_trojan_ws\n"
...
updateNginxBlog() {
	...
	if [[ "${selectInstallNginxBlogType}" =~ ^[1-6]$ ]]; then
		...
		if wget --help | grep -q show-progress; then
			wget -c -q --show-progress -P /usr/share/nginx "https://raw.githubusercontent.com/wusansan2015/v2ray-agent/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip" >/dev/null
		else
			wget -c -P /usr/share/nginx "https://raw.githubusercontent.com/wusansan2015/v2ray-agent/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip" >/dev/null
		fi
...
setDokodemoDoorUnblockNetflixOutbounds() {
	...
	if [[ -n "${setIP}" ]]; then
		...
		outbounds=$(jq -r '.outbounds += [{"tag":"netflix-80","protocol":"freedom","settings":{"domainStrategy":"AsIs","redirect":"'${setIP}':32387"}},{"tag":"netflix-443","protocol":"freedom","settings":{"domainStrategy":"AsIs","redirect":"'${setIP}':32388"}}]' ${configPath}10_ipv4_outbounds.json)
	...
	if [[ -n "${configPath}" ]]; then
		...
		if [[ -n $(ls /etc/v2ray-agent/subscribe) ]]; then
			ls /etc/v2ray-agent/subscribe | while read -r email; do
				...
				echoContent yellow "在线二维码：https://com/v1/create-qr-code/?size=400x400&data=https://${currentHost}/s/${email}\n"
